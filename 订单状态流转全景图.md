...
| `PAID` | 已支付 | ✅ 已支付 | 独享订单 | 独享订单付款完成 |
| `CONFIRMED` | 已确认 | ✅ 已确认 | 通用 | 场馆管理方确认预订成功 |
...
用户创建订单
     │
     ▼
 ┌─────────┐    用户支付     ┌─────────┐   管理员确认   ┌─────────┐   管理员核销   ┌─────────┐   管理员完成   ┌─────────┐
 │ PENDING  │──────────────▶│  PAID   │─────────────▶│CONFIRMED│─────────────▶│VERIFIED │─────────────▶│COMPLETED│
 │ 待支付   │               │ 已支付   │              │ 已确认   │              │ 已核销   │              │ 已完成   │
 └─────────┘               └─────────┘              └─────────┘              └─────────┘              └─────────┘
     │                          │                        │
     │ 用户取消                  │ 用户取消                │ 用户取消(有条件)
...
- `CONFIRMED → EXPIRED`：预约时间已过但未核销，定时任务自动过期
...
| `PAID` | `CONFIRMED`(独享), `OPEN`(拼场), `CANCELLED` | 手动 | 用户/管理员 |
...
| `SHARING_SUCCESS` / `FULL` | `CONFIRMED`, `CANCELLED` | 手动 | 管理员/用户 |
| `CONFIRMED` | `VERIFIED`, `CANCELLED`, `EXPIRED` | 手动/自动 | 管理员/系统 |
...
- 主订单变为 `CONFIRMED` → SharingOrder 状态 = `CONFIRMED`
...
│                                                                              │
│  ⑥ 管理员确认 → 核销 → 完成                                                   │
│     → Order: SHARING_SUCCESS → CONFIRMED → VERIFIED → COMPLETED              │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
| 预约过期检查 | 每天 | `CONFIRMED` 状态但预约时间已过 | 订单 → `EXPIRED` |
...
| 操作 | 当前状态 | 目标状态 | 实现状态 |
|------|---------|---------|---------|
| **确认预订** | `PAID`(独享) / `SHARING_SUCCESS`(拼场) | `CONFIRMED` | ⚠️ 仅有状态转换逻辑，无管理端界面 |
| **核销到场** | `CONFIRMED` | `VERIFIED` | ⚠️ 仅有状态转换逻辑，无管理端界面 |
...
| 功能 | 说明 | 建议状态 |
|------|------|---------|
| **订单审核** | 管理方审核新订单是否可接受 | 可在 `PAID` → `CONFIRMED` 之间增加审核逻辑 |
| **核销码/签到** | 用户到场后通过扫码或输入码核销 | 复用现有 `CONFIRMED → VERIFIED` |
...
| `PAID` 已支付 | ✅ 有条件 | 拼场订单：预约前48h，退100% | - |
| `CONFIRMED` 已确认 | ✅ 有条件 | 普通订单：预约前12h，退80% | - |
| `CONFIRMED` 已确认 | ✅ 有条件 | 拼场订单：预约前24h，退80% | - |
...
stateDiagram-v2
    ...
    CONFIRMED --> VERIFIED: 管理员核销
    CONFIRMED --> CANCELLED: 用户取消(有条件)
    CONFIRMED --> EXPIRED: 预约时间过期(自动)
...
stateDiagram-v2
    ...
    SHARING_SUCCESS --> CONFIRMED: 管理员确认
    SHARING_SUCCESS --> CANCELLED: 取消
    
    CONFIRMED --> VERIFIED: 管理员核销
    CONFIRMED --> CANCELLED: 取消(有条件)
    CONFIRMED --> EXPIRED: 预约过期(自动)
...
| `CONFIRMED` 已确认 | 🟢 绿色 | `status-confirmed` |
